...
class ContextFuzzing:
    ...
    def fuzzing_process(self, x, y, budget):
        ...
        x_ctx_label_list, x_ctx_similarity_list, x_ctx_distance_list, x_ctx_grad_list, x_ctx_inc_list = [], [], [], [], []
        ...
        for idx, (original_x, ae, ground_truth_vector, ae_label) in enumerate(zip(xi, x_adv.numpy(), target.numpy(), x_adv_pred_label)):
            ...
            if idx in idxs:
                ...
                ae_gini_list.append(1 - np.sum(np.square(x_adv_pred_vector[idx])))
                ...
                while True:
                    ...
                    for idx, left_cur_idx in enumerate(left_cur_idxs):
                        if original_image_pred_label[left_cur_idx] == np.argmax(y[left_cur_idx]) and new_ae_label[idx] != np.argmax(y[left_cur_idx]):
                            ...
                            ae_distance_list.append(new_context.distance())
                            ae_time_list.append(int(time.time() - self.time_start))
                            ae_gini_list.append(1 - np.sum(np.square(new_ae_vector[idx])))
                            ae_fol_list.append(np.linalg.norm(grads.numpy()[idx]))

                            if new_context.distance() < x_ctx_distance_list[left_cur_idx]:
...
if __name__ == "__main__":
    ...
    for param_index in [2]:
        ...
        ae_data_path = base_path + "/contextfuzz_try_delta_0.01/ae/"
        ...
        time_budgets = [1800, 3600, 7200, 18000]
        seed_nums = [1800, 3600, 7200, 18000]

        for dur_idx in range(len(time_budgets)):
            time_budget = time_budgets[dur_idx]
            seed_num = seed_nums[dur_idx]
            context_fuzzing = ContextFuzzing(model=original_model, max_step_size=0.2, ep=ep, delta=delta, gc_num=5)
