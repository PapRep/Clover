    ...
    for param_index in range(4):
        ...
        for data_type in data_type_list:
            ...
            for i in range(num_input):
                ...
                ae, ae_label, fol, gini, similarity, distance, sample_prediction = fuzzer.start(seconds=duration, append='min_dist')
                ...
                if i % 1000 == 0:
                    ...
                    np.savez(ae_data_path + data_type + "_ae.npz", idx=np.array(idx_all), ae=np.array(ae_all), ae_label=np.array(ae_label_all), fol=np.array(fol_all),
                             gini=np.array(gini_all), similarity=np.array(similarity_all), distance=np.array(distance_all), sample_prediction=np.array(sample_prediction_all))
