...
class ContextFuzzing:
    def __init__(self, model, max_step_size, steps=5, ep=0.05, gc_num=5):
        self.model = model
    ...
    def fuzzing_process(self, x, y, budget):
        # 1. first stage: generate ae to initialize the global context for the second stage fuzzing
        ae_idx_list, ae_list, ae_label_list, ae_similarity_list, ae_distance_list, ae_sample_prediction_list, ae_time_list, ae_gini_list, ae_fol_list = [], [], [], [], [], [], [], [], []
        x_ctx_label_list, x_ctx_similarity_list, x_ctx_distance_list, x_gini_list, x_ctx_grad_list, x_ctx_inc_list = [], [], [], [], [], []
        ...
        for idx, (original_x, ae, ground_truth_vector, ae_label) in enumerate(zip(xi, x_adv.numpy(), target.numpy(), x_adv_pred_label)):
            ...
            x_ctx_distance_list.append(np.inf)
            ...
            if idx in idxs:
                ...
                new_context = Context(self.model, ae=np.expand_dims(ae, axis=0), ae_label=ae_label, ep=self.ep, k=20)
                ae_distance_list.append(new_context.distance())
                ...
                ae_fol_list.append(np.linalg.norm(grads.numpy()[idx]))

                # update ctx if it is an ae
                x_ctx_distance_list[idx] = new_context.distance()
                x_gini_list[idx] = new_gini
                x_ctx_inc_list[idx] += 1
        print(f"first stage: ae length: {len(ae_list)}, ae list shape: {np.array(ae_list).shape}, ae label shape: {np.array(ae_label_list).shape} ae dist shape: {np.array(x_ctx_distance_list).shape},  time: {int(time.time()-self.time_start)}")

        # 2. second stage: start fuzzing process
        while True:
            energy = self.gc_num * np.array(x_ctx_inc_list) / np.array(x_ctx_inc_list).mean()   # calculate energy
            for batch_idx in range(int(len(x)/900)):
                cur_idxs = np.array(range(batch_idx*900, (batch_idx+1)*900))
                ...
                # gc_grads_all = [list(np.array(x_ctx_grad_list)[equivalent_class_dict[(original_image_ground_truth[idx], x_ctx_label_list[idx])][0:int(round(energy[idx]))]]) for idx in cur_idxs]
                ...
                while True:
                    ...
                    for idx, left_cur_idx in enumerate(left_cur_idxs):
                        if original_image_pred_label[left_cur_idx] == np.argmax(y[left_cur_idx]) and new_ae_label[idx] != np.argmax(y[left_cur_idx]):
                            ...
                            new_context = Context(self.model, ae=np.expand_dims(x_ae[idx], axis=0), ae_label=new_ae_label[idx], ep=self.ep, k=20)
                            ae_distance_list.append(new_context.distance())
                            ae_time_list.append(int(time.time() - self.time_start))
                            ...
                            if new_gini > x_gini_list[left_cur_idx]:
                                x_ctx_label_list[left_cur_idx] = new_ae_label[idx]
                                x_ctx_distance_list[left_cur_idx] = new_context.distance()
                                x_gini_list[left_cur_idx] = new_gini
                                x_ctx_grad_list[left_cur_idx] = feedback_grad[idx]
                    ...
                    if (time.time() - self.time_start) >= budget:
                        print(f"second stage: ae length: {len(ae_list)}, ae list shape: {np.array(ae_list).shape}, ae label shape: {np.array(ae_label_list).shape} "
                              f"ae dist shape: {np.array(x_ctx_distance_list).shape}, time: {int(time.time() - self.time_start)}")
                        return np.array(ae_idx_list), np.array(ae_list), np.array(ae_label_list), np.array(ae_similarity_list), \
                               np.array(ae_distance_list), np.array(ae_sample_prediction_list), np.array(ae_time_list), np.array(ae_gini_list), np.array(ae_fol_list)
...
if __name__ == "__main__":
    ...
    for param_index in [2, 3]:
        ...
        ae_data_path = base_path + "/contextfuzz_gini/ae/"
        ...
        for dur_idx in range(len(time_budgets)):
            ...
            seed_num = seed_nums[dur_idx]
            context_fuzzing = ContextFuzzing(model=original_model, max_step_size=0.2, steps=5, ep=ep, gc_num=5)
            all_ae_idx, all_ae, all_ae_label, all_ae_similarity, all_ae_distance, all_ae_sample_prediction, all_ae_time, all_ae_gini, all_ae_fol = \
                context_fuzzing.fuzzing_process(x=x_train[:seed_num], y=y_train[:seed_num], budget=time_budget)

            print(f"Seed number: {seed_num}, length of ae: {len(all_ae)}\n")
            np.savez(ae_data_path + '/train_ae_' + str(time_budget) + '.npz', idx=all_ae_idx, ae=all_ae, ae_label=all_ae_label, similarity=all_ae_similarity, distance=all_ae_distance,
