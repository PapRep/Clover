...
if __name__ == '__main__':
    ...
    for param_index in range(4):
        ...
        data_type_list = ["train", "test"]      # train or test
            ...
            for attack_method in attack_method_list:
                ...
                while len(ae_all) < ae_total_num:
                    for i in range(round(len(data_for_ae)/batch_size)):
                        idx, ae, ae_label, fol, gini, similarity, distance, sample_prediction, gen_time = attack_tech.generate(data_for_ae[i*batch_size: (i+1)*batch_size], label_for_ae[i*batch_size: (i+1)*batch_size])
                ...
                np.savez(ae_data_path + attack_method + "_" + data_type + "_ae.npz", idx=np.array(idx_all)[0:ae_total_num], ae=np.array(ae_all)[0:ae_total_num],
                         ...
                         similarity=np.array(similarity_all)[0:ae_total_num], distance=np.array(distance_all)[0:ae_total_num],
