...
class WhiteBoxFuzzer:
    ...
    def __init__(self, network, input, ground_truth, metric=None, strategy=None, k=10, delta=0.5, class_weight=0.5, neuron_weight=0.5, lr=0.1, trail=3, decode=None):
        '''Create a fuzzer.
    ...
    def start(self, hours=0, minutes=0, seconds=0, append='meta', verbose=0, epsilon=0.01):
        ...
        :param epsilon: An small float number which indicates the radius to generate context for a given test case.
        :return:
        '''
            ...
            while True:
                ...
                while len(worklist) > 0:
                    ...
                    for _ in range(self.trail):
                        ...
                        new_input = tf.clip_by_value(new_input, clip_value_min=0, clip_value_max=1)
                        ...
                        if distance < self.delta and self.label == np.argmax(self.ground_truth) and label != np.argmax(self.ground_truth):
                            # Add created input.
                            self.archive.add(new_input, label, distance, timer.elapsed.total_seconds(), new_cov)
                            ae_list.extend(new_input)
                            ...
                            # context
                            new_context = Context(model=self.network.model, seed=np.expand_dims(self.input, axis=0), ae_from_seed=new_input, ep=epsilon, k=20)
                            moc_list.append(new_context.moc())
                            cv_list.append(new_context.cv())
                            ccv_list.append(new_context.ccv())
                            mdist_list.append(new_context.mdist())
                            mce_list.append(new_context.mce())
        ...
        # return self.archive
        return np.array(ae_list), np.array(ae_target_list), np.array(fol_list), np.array(gini_list), np.array(moc_list), np.array(cv_list), np.array(ccv_list), np.array(mdist_list), np.array(mce_list)
