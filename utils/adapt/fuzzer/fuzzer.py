...
class WhiteBoxFuzzer:
    ...
    def start(self, hours=0, minutes=0, seconds=0, append='meta', verbose=0):
                ...
                while len(worklist) > 0:
                    ...
                    for _ in range(self.trail):
                        ...
                        if self.label != new_pred_label:
                            ...
                            # context
                            new_context = Context(model=self.network.model, seed=np.expand_dims(self.input, axis=0), ae_from_seed=new_input, k=20)
        ...
        return np.array(ae_list), np.array(ae_target_list), np.array(fol_list), np.array(gini_list), np.array(similarity_list), np.array(distance_list), np.array(sample_prediction_list)
