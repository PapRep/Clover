...
class FGSM:
    ...
    def generate(self, x, y, randRate=1):
        ...
        fol_list, similarity_list, distance_list, sample_prediction_list, time_list = [], [], [], [], []
        for selected_x_ae, selected_x_ae_label in zip(selected_x_aes, ae_prediction_labels[idxs]):
            new_context = Context(self.model, ae=np.expand_dims(selected_x_ae, axis=0), ae_label=selected_x_ae_label, ep=0.01, k=20)
            similarity_list.append(new_context.similarity())
            distance_list.append(new_context.distance())
        ...
        return np.array(idxs), selected_x_aes, selected_ground_truths, np.array(fol_list), gini_array, np.array(similarity_list), np.array(distance_list), np.array(sample_prediction_list), np.array(time_list)
...
class PGD:
    ...
    def generate(self, x, y, randRate=1):
        ...
        fol_list, similarity_list, distance_list, sample_prediction_list, time_list = [], [], [], [], []
        for selected_x_ae, selected_x_ae_label in zip(selected_x_aes, ae_prediction_labels[idxs]):
            new_context = Context(self.model, ae=np.expand_dims(selected_x_ae, axis=0), ae_label=selected_x_ae_label, ep=0.001, k=20)
            similarity_list.append(new_context.similarity())
            distance_list.append(new_context.distance())
            sample_prediction_list.append(new_context.sample_prediction())
            time_list.append(time.time() - self.time_start)
