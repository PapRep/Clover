...
if __name__ == '__main__':
    ...
    for param_index in range(4):
        ...
        print(f"Generate by {ae_generation_technique}, Model architecture: {model_architecture}, dataset: {dataset_name}, num_input:{num_input}, num_classes: {num_classes}, ep: {ep}, lr: {lr}")
        ...
        for i in range(int(np.ceil(len(x_train)/batch_size))):
                ...
                while len(img_list) > 0:
                    ...
                    for _ in range(steps):
                        ...
                        if orig_index == np.argmax(labels[idx]) and gen_index != orig_index:
                            ...
                            total_sets.append((idx+batch_size*i, time.time()-time_start, fol, gen_img.numpy(), labels[idx], gini, new_context.similarity(), new_context.distance(), new_context.sample_prediction()))
            ...
            distance_all = np.array([item[7] for item in total_sets])
            ...
            np.savez(ae_data_path + '/train_ae.npz', idx=idx_all, time=time_all, ae=ae_all, ae_label=label_all, fol=fol_all, gini=gini_all,
                     similarity=np.array(similarity_all), distance=np.array(distance_all), sample_prediction=np.array(sample_prediction_all))
